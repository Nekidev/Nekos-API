...
<Tabs items={["JavaScript", "Python 3"]}>
    <Tab>
        Using the `fetch` API:
        ```js
        const res = await fetch('https://nekos.nekidev.com/api/image/random?categories=neko')
    ...
    <Tab>
        ...
        ```py
        import requests
        res = requests.get('https://nekos.nekidev.com/api/image/random?categories=neko')
